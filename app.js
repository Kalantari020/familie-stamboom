...
function computeLayout(overrideIds, headId) {
    ...
    if (activeCfg.reassignYByBO) {
      ...
      unitsWithKids.forEach(u => {
        ...
        function centerChildrenUnder(parId, partId) {
          ...
          // Sort kids by BO
          kids.sort((a, b) => (persons[a]?.birthOrder ?? 999) - (persons[b]?.birthOrder ?? 999));

          // Build units [child, partner_same_Y_or_null]
          const kidsY = pos[kids[0]].y;
          if (!kids.every(kid => Math.abs(pos[kid].y - kidsY) < 5)) return; // kids op verschillende Y — skip
          const units = kids.map(kid => {
            const partnerRels = state.relationships.filter(r => r.type === 'partner' && (r.person1Id === kid || r.person2Id === kid));
            for (const r of partnerRels) {
              const pid = r.person1Id === kid ? r.person2Id : r.person1Id;
              if (!pos[pid]) continue;
              // Partner moet NIET zelf een ander kind in deze groep zijn (vermijdt cousin-pair dubbeltelling)
              if (kids.includes(pid)) continue;
              // Pure inlaw — hoort adjacent te komen
              const pParents = (parentsOf[pid] || []).filter(p2 => pos[p2]).length;
              const kParents = (parentsOf[kid] || []).filter(p2 => pos[p2]).length;
              if (pParents > 0 && kParents > 0) continue; // beiden eigen ouders — cousin-pair, niet meetrekken
              return { child: kid, partner: pid };
            }
            return { child: kid, partner: null };
          });

          // Compute cluster width
          let slotCount = 0;
          units.forEach(u => { slotCount += (u.partner ? 2 : 1); });
          const clusterWidth = slotCount * NODE_W + Math.max(0, slotCount - 1) * H_GAP;
          ...
          const startX = pairMidX - clusterWidth / 2;

          // Verzamel ID's die al geplaatst worden om duplicatie te vermijden
          const placedIds = new Set();
          units.forEach(u => { placedIds.add(u.child); if (u.partner) placedIds.add(u.partner); });

          // Plaats elke unit + shift hun descendants (exclusief andere units/partners)
          let x = startX;
          units.forEach(u => {
            const oldChildX = pos[u.child].x;
            pos[u.child].x = x;
            const cDelta = x - oldChildX;
            // Descendants van u.child shift mee (niet andere units)
            gatherSubtree(u.child).forEach(did => {
              if (did === u.child) return;
              if (placedIds.has(did)) return; // andere unit — niet shiften
              pos[did].x += cDelta;
            });
            x += NODE_W + H_GAP;
            if (u.partner) {
              const oldPartnerX = pos[u.partner].x;
              const oldPartnerY = pos[u.partner].y;
              pos[u.partner].x = x;
              pos[u.partner].y = kidsY;
              const pDelta = x - oldPartnerX;
              const pDeltaY = kidsY - oldPartnerY;
              gatherSubtree(u.partner).forEach(did => {
                if (did === u.partner) return;
                if (placedIds.has(did)) return;
                pos[did].x += pDelta;
                pos[did].y += pDeltaY;
              });
              x += NODE_W + H_GAP;
        ...
        // Roep centerChildrenUnder aan op p1+p2 (gen1-paar) zelf.
        // De functie rebuilds de gen2-cluster (kinderen van p1+p2 met inlaws
        // adjacent) en recurses daarna naar gen3+.
        centerChildrenUnder(p1, p2);
      });

      // ── UNIVERSELE PARTNER-SYNC: alle descendants incl. gen3+ ──
      // Voor elke persoon in de tree: als partner op andere Y staat (< NODE_H diff),
      // plaats partner op zelfde Y naast hen. Fixt nieuw-toegevoegde partners
      // (zoals Zaki Mirzei naast Khadija) automatisch.
      const syncedPairs = new Set();
      Object.keys(pos).forEach(personId => {
        const pRels = state.relationships.filter(r => r.type === 'partner' && (r.person1Id === personId || r.person2Id === personId));
        for (const r of pRels) {
          const partnerId = r.person1Id === personId ? r.person2Id : r.person1Id;
          if (!pos[partnerId]) continue;
          const pairKey = [personId, partnerId].sort().join(',');
          if (syncedPairs.has(pairKey)) continue;
          syncedPairs.add(pairKey);
          const dy = Math.abs(pos[partnerId].y - pos[personId].y);
          if (dy < 0.5) continue; // al op zelfde Y
          if (dy >= 2 * (NODE_H + V_GAP)) continue; // verschillende generaties — niet aanraken
          // Determine pure inlaw (parent without parents in tree)
          const aPar = (parentsOf[personId] || []).filter(pid => pos[pid]).length;
          const bPar = (parentsOf[partnerId] || []).filter(pid => pos[pid]).length;
          let inlaw, anchor;
          if (aPar === 0 && bPar > 0) { inlaw = personId; anchor = partnerId; }
          else if (bPar === 0 && aPar > 0) { inlaw = partnerId; anchor = personId; }
          else continue; // geen duidelijke inlaw
          // Sync inlaw naar anchor's Y, plaats naast anchor
          const oldY = pos[inlaw].y;
          const oldX = pos[inlaw].x;
          pos[inlaw].y = pos[anchor].y;
          pos[inlaw].x = pos[anchor].x + NODE_W + H_GAP;
          const dX = pos[inlaw].x - oldX;
          const dY = pos[inlaw].y - oldY;
          // Shift inlaw's descendants mee (exclusief anchor's subtree)
          const anchorSub = new Set();
          const anchorQ = [anchor];
          while (anchorQ.length) {
            const id = anchorQ.shift();
            if (anchorSub.has(id)) continue;
            anchorSub.add(id);
            (childrenOfMap[id] || []).forEach(cid => { if (pos[cid] && !anchorSub.has(cid)) anchorQ.push(cid); });
          }
          const inlawSub = new Set();
          const inlawQ = [inlaw];
          while (inlawQ.length) {
            const id = inlawQ.shift();
            if (inlawSub.has(id)) continue;
            inlawSub.add(id);
            (childrenOfMap[id] || []).forEach(cid => { if (pos[cid] && !inlawSub.has(cid)) inlawQ.push(cid); });
          }
          inlawSub.delete(inlaw);
          inlawSub.forEach(did => {
            if (anchorSub.has(did)) return;
            pos[did].x += dX;
            pos[did].y += dY;
          });
        }
  ...
  if (headId === 'pmndyrysy3eq7') (function rebuildSayedahmedGen1() {
    ...
    const compactLeafsToY = [
      ...
      { leafIds: ['pmo4u1b0rwt71', 'pmo4u2pw7zjnf'], targetY: 744 }, // Fereshta's kid Hasanaat + Mariam's kid Abu Bakr op Iqra's Y
    ];
  ...
  if (headId) {
    ...
    state.persons.forEach(p => {
      ...
      if (typeof window !== 'undefined' && window._debugCenter) window._debugCenter.push({ p: p.id, dx, kids: kids.length });
    });
  }
...
(function init() {
  ...
  // snapshots_v639.json: 93 stambomen, 7 unapproved (Ahmad Saidi/Allahmahmad/Fazelahmad/
  // Ilas/Mahmadgul/Sayedahmed/Waheed) houden v633 positions tot Hakim opnieuw goedkeurt.
  fetch('snapshots_v639.json?v=' + APP_VERSION)
    .then(r => r.json())
    .then(snaps => {
